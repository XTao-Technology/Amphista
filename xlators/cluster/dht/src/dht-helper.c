...
void
dht_refresh_unwind(call_frame_t *frame)
{
        ...
        switch (local->fop) {
                ...
                case GF_FOP_RMDIR:
                        ...
                        break;
                case GF_FOP_SYMLINK:
                        DHT_STACK_UNWIND (symlink, frame, -1, op_errno,
                                        NULL, NULL, NULL, NULL, NULL);
                case GF_FOP_LINK:
                ...
                case GF_FOP_SETXATTR:
                        ...
                        break;
                default:
                        gf_msg_callingfn ("dht", GF_LOG_ERROR, EINVAL,
...
int
dht_refresh_frame_clean(call_frame_t * frame, gf_boolean_t attempt_wind)
{
        int                        ret = -1;
...
int
dht_refresh_parent_layout_done (call_frame_t *frame, int op_errno)
{
        ...
        local = frame->local;
        if(local->refresh_frame){
           main_frame = local->refresh_frame;
           local->refresh_frame = NULL;
        }

        if(main_frame && main_frame->local){
             main_local = main_frame->local;
             main_local->op_errno = op_errno;
        }

        if(main_local && main_local->stub){
           stub = main_local->stub;
           main_local->stub = NULL;
        }
...
int
dht_lookup_parent_cbk (call_frame_t *frame, void *cookie, xlator_t *this,
                       ...
                       struct iatt *postparent)
{
        ...
        int      this_call_cnt      = 0;
        call_frame_t *refresh_frame = NULL;
...
unlock:
        ...
        if(is_last_call(this_call_cnt)){
                if (local->op_ret == 0) {
                        ...
                        gf_log(this->name, GF_LOG_DEBUG, "layout set gfid = %s layout->gen = %d", uuid_utoa(local->inode->gfid), layout->gen);
                        dht_layout_set (this, local->inode, layout);
                }
                refresh_frame = local->refresh_frame;
                local->refresh_frame = NULL;
                dht_refresh_frame_clean (refresh_frame, _gf_true);
                DHT_STACK_DESTROY (frame);
        }
        ...
        return ret;
out:
        dht_refresh_parent_layout_done (frame, local->op_errno);
        return ret;
}

int
dht_refresh_parent_layout (xlator_t *this, call_frame_t *frame)
{
        int           ret     = -1;
        int          call_cnt = 0;
        int             i     = 0;
        ...
        if (ret) {
                gf_msg (this->name, GF_LOG_WARNING, ENOMEM,
                        ...
                        "gfid %s", conf->xattr_name, uuid_utoa(loc.inode->gfid));
                goto err;
        }
        ...
        for (i = 0; i < call_cnt; i++) {
                STACK_WIND (frame, dht_lookup_parent_cbk,
                            conf->subvolumes[i],
...
/*
 * return values
 * 0    lookup healed parent layout
 * -1   lookup failed
 */
int
dht_layout_refreshing (xlator_t *this,  loc_t * loc,
                        call_frame_t * refresh_frame)
{
        ...
        if(ret){
                gf_log(this->name, GF_LOG_ERROR, "refresh parent gfid: %slayout failed ", uuid_utoa(tmp_loc.inode->gfid));
                goto clean;
        }
