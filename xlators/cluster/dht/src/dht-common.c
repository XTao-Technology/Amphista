...
int
dht_aggregate_dirstat_xattr(dict_t *dst, char *key, data_t *value)
{
        int ret = -1;
        ...
        ret = dict_get_bin(dst, key, (void **)&stbuf);
        if (ret < 0) {
                stbuf = GF_CALLOC(1, sizeof(struct iatt), gf_common_mt_char);
                if (stbuf == NULL) {
                        gf_log("dht", GF_LOG_WARNING, "Memory allocation failed!");
                        ret = -1;
                        goto out;
                }
                dht_iatt_merge(NULL, stbuf, (struct iatt *)value->data, NULL);
                ret = dict_set_bin(dst, key, stbuf, sizeof(struct iatt));
                if (ret < 0) {
...
dht_aggregate (dict_t *this, char *key, data_t *value, void *data)
{
        ...
        if (strcmp (key, QUOTA_SIZE_KEY) == 0) {
                ...
                if (ret < 0)
                        goto out;
        } else if (strcmp (key, DIR_STAT_CACHE) == 0) {
                ret = dht_aggregate_dirstat_xattr(dst, key, value);
                if (ret) {
                        gf_log("dht", GF_LOG_WARNING, "Failed to aggregate dirstat xattr");
                        goto out;
                }
        }else {
        ...
        if (key && (strcmp (key, GF_XATTR_LINKINFO_KEY) == 0)) {
                ...
                if (!hashed_subvol) {
                        gf_msg (this->name, GF_LOG_ERROR, 0,
                                ...
                                loc->path);
                        local->stub = fop_getxattr_stub (frame, dht_getxattr, loc, key, xdata);
                        if(!local->stub){
                            op_errno = ENOMEM;
                            goto err;
                        }
              ...
              loc_t *loc, dict_t *xattr, int flags, dict_t *xdata)
{
        ...
        if (tmp) {
                ...
                if (!local->rebalance.target_node) {
                        ...
                        local->stub = fop_setxattr_stub (frame, dht_setxattr, loc, xattr, flags, xdata);
                        if(!local->stub){
                                op_errno = ENOMEM;
                                goto err;
                        }
        ...
        LOCK (&frame->lock);
        {
                ...
                if (xdata) {
                        if (local->xattr == NULL) {
                                ...
                                dht_aggregate_xattr (local->xattr, xdata);
                        }
                }
        }
unlock:
           ...
           loc_t *loc, mode_t mode, dev_t rdev, mode_t umask, dict_t *params)
{
        ...
        if (!subvol) {
                ...
                local->stub = fop_mknod_stub(frame, dht_mknod, loc, mode, rdev, umask, params);
                if(!local->stub){
                        op_errno = ENOMEM;
                        goto err;
                }
             ...
             const char *linkname, loc_t *loc, mode_t umask, dict_t *params)
{
        ...
        if (!subvol) {
                ...
                local->stub = fop_symlink_stub (frame, dht_symlink, linkname, loc, umask, params);
                if(!local->stub){
                        op_errno = ENOMEM;
                        goto err;
                }
            ...
            dict_t *xdata)
{
        ...
        cached_subvol = local->cached_subvol;
        local->hashed_subvol = dht_subvol_get_hashed (this, &local->loc);
        if (!cached_subvol || !local->hashed_subvol) {
                gf_msg_debug (this->name, 0,
                              "no cached subvolume for path=%s", loc->path);

                local->stub = fop_unlink_stub (frame, dht_unlink, loc, xflag, xdata);
                if(!local->stub){
                    op_errno = ENOMEM;
                    goto err;
                }
          ...
          loc_t *oldloc, loc_t *newloc, dict_t *xdata)
{
        ...
        if (!cached_subvol) {
                ...
                local->stub = fop_link_stub (frame, dht_link, oldloc, newloc, xdata);
                if(!local->stub){
                        op_errno = ENOMEM;
                        goto err;
                }

                ret = dht_layout_refreshing (this, oldloc, frame);
                if(ret){
                        gf_log (this->name, GF_LOG_ERROR, "refresh layout failed");
                        op_errno = ENOMEM;
                        goto err;
        ...
        if (!hashed_subvol) {
                ...
                ret = dht_layout_refreshing (this, newloc, frame);
                if(ret){
                        gf_log (this->name, GF_LOG_ERROR, "refresh layout failed");
                        op_errno = ENOMEM;
                        goto err;
            ...
            mode_t umask, fd_t *fd, dict_t *params)
{
        ...
        if (!subvol) {
                ...
                local->stub = fop_create_stub (frame, dht_create, loc,
           ...
           loc_t *loc, mode_t mode, mode_t umask, dict_t *params)
{
        ...
        if (hashed_subvol == NULL) {
                ...
                local->stub = fop_mkdir_stub (frame, dht_mkdir, loc, mode, umask, params);
                if(!local->stub){
                        op_errno = ENOMEM;
                        goto err;
                }
           ...
           dict_t *xdata)
{
        ...
        if(!local->hashed_subvol){
                ...
                local->stub = fop_rmdir_stub (frame, dht_rmdir, loc, flags, xdata);
                if(!local->stub){
                        op_errno = ENOMEM;
                        goto err;
                }
