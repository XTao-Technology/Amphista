               ...
               loc_t *loc, fd_t *fd, dict_t *xdata)
{
        ...
        if (dict_get(xdata, NFS_SVC_READDIR)) {
                struct iatt *stbuf = NULL;

                stbuf = GF_CALLOC(1, sizeof(struct iatt), gf_common_mt_char);
                if (!stbuf) {
                        gf_log(this->name, GF_LOG_ERROR, "allocate rsp iatt for posix readdir failed!");
                        goto out;
                }

                op_ret = posix_fdstat (this, pfd->fd, stbuf);
                if (op_ret == -1) {
                        op_errno = errno;
                        gf_msg (this->name, GF_LOG_ERROR, errno, P_MSG_FSTAT_FAILED,
                ...
                xattr_rsp = dict_new();
                if (xattr_rsp == NULL) {
                        gf_log(this->name, GF_LOG_ERROR, "allocate dict for posix readdir rsp xattr failed!");
                        GF_FREE(stbuf);
                        goto out;
                }
                op_ret = dict_set_bin(xattr_rsp, DIR_STAT_CACHE, stbuf, sizeof(struct iatt));
                if (op_ret) {
                        gf_log(this->name, GF_LOG_ERROR, "set nfs readdir iatt key failed!");
                        dict_unref(xattr_rsp);
                        GF_FREE(stbuf);
                        goto out;
                }
...
out:
        ...
        SET_TO_OLD_FS_ID ();
        STACK_UNWIND_STRICT (opendir, frame, op_ret, op_errno, fd, xattr_rsp);
        if (xattr_rsp) {
                dict_unref(xattr_rsp);
        }
        return 0;
