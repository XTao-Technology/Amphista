                                ...
                                gf_boolean_t *identical)
{
	...
	uint32_t                cksum2 = 0;
	xlator_t                *this = NULL;
	char    file1_md5buf[MD5_STR_LEN + 1] ={0};
	char    file2_md5buf[MD5_STR_LEN + 1] ={0};
	...
	ret = compute_file_md5_for_path(filename1, file1_md5buf);
	if (ret)
		goto out;

	ret = compute_file_md5_for_path(filename2, file2_md5buf);
	if (ret)
		goto out;

	if(strncmp(file1_md5buf, file2_md5buf, MD5_STR_LEN) == 0){
		*identical = _gf_true;
	}else{
                *identical = _gf_false;
        }
