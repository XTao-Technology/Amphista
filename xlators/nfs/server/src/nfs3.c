                    ...
                    struct iatt *postparent, dict_t *xdata)
{
        ...
        if (oldinode != inode) {
                gf_log (GF_NFS3, GF_LOG_INFO, "nfs create inode is exist and refresh loc inode");
                if (cs->resolvedloc.inode && cs->resolvedloc.inode == inode)
                        inode_unref (cs->resolvedloc.inode);

                cs->resolvedloc.inode = inode_ref (oldinode);
        }
...
nfs3_readdir_open_resume (void *carg)
{
         ...
         */
        if (cs->cookie == 0) {
                gf_log("nfs-server", GF_LOG_DEBUG, "nfs server start to opendir...");
                nfs_request_user_init (&nfu, cs->req);
              ...
              uint64_t cverf, count3 dircount, count3 maxcount)
{
        ...
        cs->parent = *fh;
        if (cs->cookie == 0) {
		gf_log("nfs-server", GF_LOG_DEBUG, "nfs server start to readdir...");
	}
