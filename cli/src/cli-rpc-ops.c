...
static int
print_quota_list_from_glusterd (int index, cli_local_t *local, char *default_sl,
                                  dict_t *dict, char *path, int type)
{
        int64_t         avail            = 0;
        char            percent_str[20]  = {0};
        char           *sl_final         = NULL;
        ...
        ret = dict_get_int32(dict, key_resp_limit_set, &limit_flag);
        ret |= dict_get_int32(dict, key_resp_code, &errcode);
        if (ret) {
            gf_log ("cli", GF_LOG_ERROR, "Failed to get result or limit_set from rsp_dict"
                        "on path%d.",index );
            goto out;
        }

        if (errcode) {
            ret = dict_get_str (dict, key_resp_msg, &err_msg);
            if (ret) {
                gf_log ("cli", GF_LOG_ERROR, "Failed to get err_msg from resp.err_msg"
                        "on path%d.",index );
                goto out;
            }
            cli_err("%s",err_msg);
        ...
        ret |= dict_get_int64 (dict,key_used_size, &used_space.size);
        ret |= dict_get_int64 (dict,key_used_file_count, &used_space.file_count);
        ret |= dict_get_int64 (dict,key_used_dir_count, &used_space.dir_count);
        if (ret) {
            gf_log ("cli", GF_LOG_ERROR, "Failed to get used_space from rsp_dict"
                        "on path%d.",index );
            goto out;
        }
        ...
        if (limit_set) {
            ...
            ret |= dict_get_int64 (dict,key_limits_hl, &limits.hl);
            ret |= dict_get_int64 (dict,key_limits_sl, &limits.sl);
            if (ret) {
                gf_log ("cli", GF_LOG_ERROR, "Failed to get quota_limits from rsp_dict"
                        "on path%d.",index );
                goto out;
            }
            limits.hl = ntoh64 (limits.hl);
        ...
        ret = print_quota_list_output (local, path, default_sl, &limits, &used_space, type, limit_set);
out:
        return ret;
}
                                   ...
                                   int op_ret, int op_errno, char *op_errstr)
{
...
#else   //since docker version, support quota list from glusterd in docker
    ...
    while (count--){
        ...
        ret = print_quota_list_from_glusterd (i,local, default_sl, dict, path, type);
