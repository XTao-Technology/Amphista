...
int compute_file_md5_for_path(char * path, char * md5_str)
{
	...
	int                                       i   = -1;
	MD5_CTX                                   md5 = {0};
	unsigned char md5_buf[GF_CHECKSUM_BUF_SIZE]   = {0};
	...
	while(1){
		ret = read (fd, &md5_buf, GF_CHECKSUM_BUF_SIZE);
		if (-1 == ret)
		{
			gf_log(THIS->name, GF_LOG_ERROR, "read error");
			goto out;
		}
		MD5_Update(&md5, md5_buf, ret);
		if (0 == ret || ret < GF_CHECKSUM_BUF_SIZE)
			break;
	}
