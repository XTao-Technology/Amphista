...
int compute_file_md5_for_path(char * path, char * md5_str);
...
#ifdef PERF_STATS
#define STATS_INC(counter) atomic_add_and_fetch(&(counter), 1)
#define STATS_DEC(counter) atomic_sub_and_fetch(&(counter), 1)
#define STATS_ADD(counter, value) atomic_add_and_fetch(&(counter), value)

#else

#define STATS_INC(counter)
#define STATS_ADD(counter, value)
#define STATS_DEC(counter)
